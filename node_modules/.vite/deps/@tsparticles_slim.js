...
function drawEmoji(data, image) {
  const { context, opacity } = data, half7 = 0.5, previousAlpha = context.globalAlpha;
  if (!image) {
    return;
  }
  const diameter = image.width, radius = diameter * half7;
...
var EmojiDrawer = class {
  ...
  particleInit(_container, particle) {
    const double8 = 2, shapeData = particle.shapeData;
    ...
    const padding = emojiOptions.padding * double8, maxSize = getRangeMax(particle.size.value), fullSize = maxSize + padding, canvasSize = fullSize * double8;
    let image;
    if (typeof OffscreenCanvas !== "undefined") {
      ...
      context.font = `400 ${maxSize * double8}px ${font}`;
  ...
  constructor(container, engine) {
    ...
    this._clickBubble = () => {
      ...
      for (const particle of query) {
        ...
        if (timeSpent > bubbleOptions.duration * double2) {
    ...
    this._singleSelectorHover = (delta, selector, div) => {
      ...
      selectors.forEach((item) => {
        ...
        }, repulseRadius = elem.offsetWidth * half2 * pxRatio, area = div.type === DivType.circle ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container2.particles.quadTree.query(area, (p) => this.isEnabled(p));
        for (const particle of query) {
  ...
  constructor(engine, container) {
    ...
    this._singleSelectorRepulse = (selector, div) => {
      ...
      query.forEach((item) => {
        ...
        }, repulseRadius = elem.offsetWidth * half3 * pxRatio, area = div.type === DivType.circle ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), divs = repulse.divs, divRepulse = divMode(divs, elem);
...
function drawGif(data) {
  ...
  const pos = { x: -image.gifData.width * half4, y: -image.gifData.height * half4 }, frame = image.gifData.frames[frameIndex];
...
var ParallaxMover = class {
  ...
  move(particle) {
    ...
    const canvasSize = container.canvas.size, canvasCenter = {
      x: canvasSize.width * half5,
      y: canvasSize.height * half5
...
var Attractor2 = class extends ParticlesInteractorBase {
  ...
  interact(p1) {
    ...
    for (const p2 of query) {
      ...
      const pos2 = p2.getPosition(), { dx, dy } = getDistances(pos1, pos2), rotate = p1.options.move.attract.rotate, ax = dx / (rotate.x * attractFactor), ay = dy / (rotate.y * attractFactor), p1Factor = p2.size.value / p1.size.value, p2Factor = identity3 / p1Factor;
...
var Collider = class extends ParticlesInteractorBase {
  ...
  interact(p1, delta) {
    ...
    const container = this.container, pos1 = p1.getPosition(), radius1 = p1.getRadius(), query = container.particles.quadTree.queryCircle(pos1, radius1 * double4);
    for (const p2 of query) {
...
function drawPolygon(data, start, side) {
  ...
  context.moveTo(origin2.x, origin2.y);
  for (let i = 0; i < sideCount; i++) {
    context.lineTo(side.length, origin2.y);
    context.translate(side.length, origin2.y);
...
function drawSquare(data) {
  const { context, radius } = data, fixedRadius = radius / fixFactor, fixedDiameter = fixedRadius * double7;
...
function drawStar(data) {
  ...
  context.moveTo(origin3.x, origin3.y - radius);
  for (let i = 0; i < sides5; i++) {
    context.rotate(Math.PI / sides5);
    context.lineTo(origin3.x, origin3.y - radius * inset);
    context.rotate(Math.PI / sides5);
    context.lineTo(origin3.x, origin3.y - radius);
  }
}
